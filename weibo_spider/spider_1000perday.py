...
def unformatLimitTime(limit_time):
    if limit_time[(-2)] == '0':
        ...
        if len(limit_time) == 13:
            limit_time = limit_time[:-3] + '-' + limit_time[-2:]
        else:
            limit_time = limit_time[:-2] + '-' + limit_time[(-1)]
    return limit_time
...
class NewSuperWeiboTimelineTopicSpider(object):
    ...
    def __init__(self, keyword, start_time, end_time, cookies=None, only_origin=False):
        ...
        self.result_file = self.topic_folder + '/' + '2020疫情_1000perday_new' + '.csv'
    ...
    def parseWeibo(self, html):
        ...
        for weibo in weibos:
            ...
            try:
                content = \
                    weibo.xpath('.//div[@class="content"]/p[@class="txt" and @node-type="feed_list_content_full"]')[
                        0].xpath('string(.)').strip()
            except:
                content = weibo.xpath('.//div[@class="content"]/p[@class="txt" and @node-type="feed_list_content"]')[
                    0].xpath('string(.)').strip()
            ...
            aweibo = {'mid': mid,
                      ...
                      'like_num': like_num}
            # (publish_time, mid, user_name, user_link, content.encode('GBK', 'ignore').decode('GBK'), image_urls, weibo_link, forward_num, comment_num, like_num)
            yield aweibo
    ...
    def crawl(self):
        ...
        while True:
            ...
            while current_page <= self.max_page:
                ...
                try:
                    response = requests.get('https://s.weibo.com/weibo', headers=(self.headers), params=(self.params),
            ...
            self.total_written_num += self.got_weibos_num
            if self.got_weibos_num > self.written_weibos_num:
                self.write_csv()
            try:
                # print(self.got_weibos[(-1)]['publish_time'])
            ...
            if earliest_time > self.start_time:
                if self.end_time[:10] == earliest_time[:10]:
                    self.end_time = earliest_time[:10] + '-0'
                else:
